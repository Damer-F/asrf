...
def predict(
    ...
    image_dir = os.path.join(result_path, "predictions")
    array_dir = os.path.join(result_path, "prediction_arrays")
    ...
    with torch.no_grad():
        for sample in loader:
            ...
            np.save(os.path.join(array_dir, name[:-4] + "_pred.npy"), pred[0])
            np.save(
                os.path.join(array_dir, name[:-4] + "_refined_pred.npy"),
                refined_pred[0],
            )
            np.save(os.path.join(array_dir, name[:-4] + "_gt.npy"), t[0])

            # make graph for boundary regression
            output_bound = output_bound[0, 0]
            np.save(os.path.join(array_dir, name[:-4] + "_boundary.npy"), output_bound)
            ...
            plt.savefig(os.path.join(image_dir, name[:-4] + "_boundary.png"))
            plt.close(fig)
