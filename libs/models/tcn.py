...
class DilatedResidualLayer(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        x : input [B, C, T] 
        '''
        out = F.relu(self.conv_dilated(x)) # 扩大感受野，一维 Conv_dilated 捕捉时序上各个图的像素相关性
        out = self.conv_in(out) # 跨通道 Conv，针对各个图自身获取特征
...
class ActionSegmentRefinementFramework(nn.Module):
    ...
    def __init__(
        ...
        **kwargs: Any
        ...
        # 从小到大 超大空洞卷积 一步步 扩大 空洞范围，捕捉时空相关性
        shared_layers = [
            DilatedResidualLayer(2 ** i, n_features, n_features)
            for i in range(n_layers)
        ]
        ...
        asb = [
            # in_channel --- > n_classes， asb 处理的是类别  [B, num_classes, T]
            SingleStageTCN(n_classes, n_features, n_classes, n_layers)
